...
md`<div style="color: grey; font: 13px/25.5px var(--sans-serif); text-transform: uppercase;"><h1 style="display: none;">Stacked bar chart</h1><a href="https://d3js.org/">D3</a> › <a href="/@d3/gallery">Gallery</a></div>
...
legend({color: chart.scales.color, title: "Score (0-10)"})
)}
...
function _chart(d3,data)
{
  ...
  const x = d3.scaleBand()
      .domain(d3.groupSort(data, D => -d3.sum(D, d => d.totalScore), d => d.student))
...
async function _data(FileAttachment) {
  ...
  return data.map(d => ({
      student: d.姓名,
      totalScore: d.作業一 + d.作業二 + d.作業三 + d.作業四 + d.作業五 + d.作業六 + d.作業七 + d.作業八 + d.作業九 + d.作業十
  }));
}
