...
Inputs.color({ label: "background color", value: "#FFECF5"})
)}
...
(width, height, offsetX, offsetY, scale) => {
  ...
  const maxValue = 42;
  const thresholds = d3.range(0, maxValue + 1, maxValue / 5);
  const colorRange = d3.schemePastel2;
  const thresholdScale = d3.scaleThreshold().domain(thresholds).range(colorRange);
  // const colorRange = thresholds.map(value => d3.interpolatePurples(value / maxValue));
  // const thresholdScale = d3.scaleThreshold().domain(thresholds).range(colorRange);
